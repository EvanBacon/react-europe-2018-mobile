...
class CameraModule {
  ...
  async onCapabilitiesReady(track) {
    ...
    if (capabilities.focusMode) {
      constraints.focusMode = CapabilityUtils.convertAutoFocusJSONToNative(
        this.autoFocus
      );
    }
    if (capabilities.torch) {
      constraints.torch = CapabilityUtils.convertFlashModeJSONToNative(
        this.flashMode
      );
    }
  ...
  async _syncTrackCapabilities() {
    if (this.stream) {
      await Promise.all(
        this.stream.getTracks().map(track => this.onCapabilitiesReady(track))
      );
    }
  }
      ...
      if (callback && this.webWorker) {
        if (event && Array.isArray(event.data)) {
          ...
          for (const result of event.data) {
            if (context) {
              Utils.drawBarcodeBounds(
                context,
                result.location,
                this.drawBarcodeOptions
              );
            }
            callback(result);
          }
  ...
  takePicture(config) {
    ...
    if (config.onPictureSaved) {
      config.onPictureSaved({
        nativeEvent: {data: capturedPicture, id: config.id},
      });
    }
