...
function isIOS() {
  const isIOSUA = /(iPad|iPhone|iPod)/g.test(global.navigator.userAgent);
  const isIE11 = !!global.window['MSStream'];
...
export function getIdealConstraints(preferredCameraType, width, height) {
  ...
  if (
    preferredCameraType &&
    Object.values(CameraType).includes(preferredCameraType)
  ) {
    const mode = CameraTypeToFacingMode[preferredCameraType];
    if (isIOS()) {
